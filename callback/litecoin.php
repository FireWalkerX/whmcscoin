...
while($result = mysql_fetch_array($results)){
        $ltc_info = select_query("litecoin_info", "secret", array("invoice_id"=>$result['invoice_id']), "invoice_id", "DESC", 1);
	...
	$received = $litecoin->listtransactions($ltc_info['secret'], 1000); # I feel like we can do better than just get the LAST ONE THOUSAND TRANSACTIONS but we'll figure that out later
        foreach ($received as $recArr) {
            ...
            insert_query('litecoin_payments', array("invoice_id"=>$result['invoice_id'],"amount"=>$recArr['amount'],"address"=>$recArr['address'],"confirmations"=>$recArr['confirmations']));
            addInvoicePayment($result['invoice_id'], $recArr['txid'], $amount_received, 0, $gatewaymodule);
